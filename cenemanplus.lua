...
-- ===================== FEATURES KODU (BİREBİR ATILAN DOSYADAN) =====================
local features = {} -- (tüm kod buraya yapıştırıldı, eksiksiz)
...
do
    ...
    P:AddToggle("SpeedToggle", {
        Title = "Speed Hack",
        Default = false,
        Callback = function(v) features.ToggleSpeed(v) end
    })

    P:AddSlider("SpeedSlider", {
        Title = "Walk Speed",
        Min = 16,
        Max = 500,
        Default = 150,
    ...
    P:AddToggle("FlyToggle", {
        Title = "Fly (WASD + Space/LCtrl)",
        Default = false,
        Callback = function(v) features.ToggleFly(v) end
    })

    P:AddToggle("NoclipToggle", {
        Title = "Noclip",
        Default = false,
        Callback = function(v) features.ToggleNoclip(v) end
    })

    P:AddToggle("InfJumpToggle", {
        Title = "Infinite Jump",
        Default = false,
        Callback = function(v) features.ToggleInfiniteJump(v) end
    })

    P:AddToggle("GodmodeToggle", {
        Title = "Godmode (Ultra Hard Hook)",
        Default = false,
        Callback = function(v) ToggleGodmode(v) end
    })

    P:AddToggle("HardInvisibleToggle", {
        Title = "Hard Invisible (E ile toggle)",
        Default = false,
        Callback = function(v) features.ToggleHardInvisible(v) end
    })

    P:AddToggle("MyTinyHitboxToggle", {
        Title = "My Tiny Hitbox (Sadece Sen)",
        Default = false,
        Callback = function(v) features.ToggleMyTinyHitbox(v) end
    })
...
do
    ...
    C:AddToggle("AimbotToggle", {
        Title = "Aimbot (Açı Tabanlı)",
        Default = false,
        Callback = function(v) features.ToggleAimbot(v) end
    })

    C:AddSlider("SmoothnessSlider", {
        Title = "Aimbot Smoothness (1 = instant)",
        Min = 1,
        Max = 10,
        Default = 2,
        Rounding = 1,
        Callback = function(v) features.Smoothness = v end
    })

    C:AddToggle("TriggerBotToggle", {
        Title = "TriggerBot (Auto Fire)",
        Default = false,
        Callback = function(v) features.TriggerOnAim = v end
    })

    C:AddToggle("HeadshotRedirectToggle", {
        Title = "Headshot Redirect",
        Default = false,
        Callback = function(v) features.ToggleHeadshotRedirect(v) end
    })

    C:AddToggle("MultiHookToggle", {
        Title = "Multi-Hook Silent (En Güçlü)",
        Default = false,
        Callback = function(v) features.ToggleMultiHook(v) end
    })

    C:AddToggle("KillAuraToggle", {
        Title = "Kill Aura (20 stud + DamageRemote)",
        Default = false,
        Callback = function(v) features.ToggleKillAura(v) end
    })

    C:AddSlider("DamageSlider", {
        Title = "Kill Aura Damage",
        Min = 100,
        Max = 9000,
        Default = 900,
        Rounding = 1,
        Callback = function(v) features.DamageAmount = v end
    })
end
...
do
    ...
    R:AddToggle("SilentAimToggle", {
        Title = "Silent Aim",
        Default = false,
        Callback = function(v) ToggleSilentAim(v) end
    })

    R:AddToggle("RageModeToggle", {
        Title = "Rage Mode (Duvar Arkası)",
        Default = false,
        Callback = function(v) ToggleRageMode(v) end
    })

    R:AddToggle("RapidFireToggle", {
        Title = "Rapid Fire / No Cooldown",
        Default = false,
        Callback = function(v) ToggleFireRate(v) end
    })

    R:AddToggle("EnemyBigHitboxToggle", {
        Title = "Enemy Big Hitbox (15x15)",
        Default = false,
        Callback = function(v) features.ToggleEnemyBigHitbox(v) end
    })

    R:AddToggle("TinyHitboxToggle", {
        Title = "Enemy Tiny Hitbox (Hard Hook)",
        Default = false,
        Callback = function(v) features.ToggleTinyHitbox(v) end
    })

    R:AddToggle("AutoBehindToggle", {
        Title = "Auto TP Behind Enemy",
        Default = false,
        Callback = function(v) features.ToggleAutoBehind(v) end
    })
end
...
do
    ...
    V:AddToggle("SkeletonToggle", {
        Title = "Skeleton ESP (Rainbow)",
        Default = false,
        Callback = function(v) features.ToggleSkeleton(v) end
    })

    V:AddToggle("BoxToggle", {
        Title = "Box ESP (Rainbow)",
        Default = false,
        Callback = function(v) features.ToggleBox(v) end
    })

    V:AddToggle("RainbowNameToggle", {
        Title = "Rainbow Name ESP",
        Default = false,
        Callback = function(v) features.ToggleRainbowName(v) end
    })

    V:AddToggle("GlowToggle", {
        Title = "Glow / Highlight ESP",
        Default = false,
        Callback = function(v) features.ToggleGlow(v) end
    })

    V:AddToggle("TracersToggle", {
        Title = "Tracers (Rainbow)",
        Default = false,
        Callback = function(v) features.ToggleTracers(v) end
    })

    -- Bonus: 3D Box Preview (önceki isteğin üzerine)
    V:AddToggle("3DBoxPreview", {
        Title = "3D Box Preview (Özel)",
        Default = true,
        Callback = function(v) ESPSettings.Boxes3D = v end
    })
end
...
do
    ...
    M:AddToggle("TeleportMouseToggle", {
        Title = "Teleport to Mouse (T tuşu)",
        Default = false,
        Callback = function(v) features.ToggleTeleport(v) end
    })

    M:AddParagraph({Title = "Emote Packs"})

    local EmoteDropdown = M:AddDropdown("EmotePack", {
        Title = "Emote Pack Seç",
        Values = {"Default", "Zombie", "Ninja", "Elder", "Vampire", "Astronaut", "Pirate", "Superhero", "Bubbly", "Mage", "Robot", "Toy", "Werewolf", "Moderation"},
        Multi = false,
        Default = "Default",
        Callback = function(value)
            if value == "Default" then features.ToggleDefault(true)
            elseif value == "Zombie" then features.ToggleZombie(true)
            elseif value == "Ninja" then features.ToggleNinja(true)
            elseif value == "Elder" then features.ToggleElder(true)
            elseif value == "Vampire" then features.ToggleVampire(true)
            elseif value == "Astronaut" then features.ToggleAstronaut(true)
            elseif value == "Pirate" then features.TogglePirate(true)
            elseif value == "Superhero" then features.ToggleSuperhero(true)
            elseif value == "Bubbly" then features.ToggleBubbly(true)
            elseif value == "Mage" then features.ToggleMage(true)
            elseif value == "Robot" then features.ToggleRobot(true)
            elseif value == "Toy" then features.ToggleToy(true)
            elseif value == "Werewolf" then features.ToggleWerewolf(true)
            elseif value == "Moderation" then features.ToggleModeration(true)
            end
            Fluent:Notify({Title = "MYLF", Content = value.." emote pack uygulandı!", Duration = 4})
        end
    })
...
Fluent:Notify({
    Title = "MYLF | Hub ⚡",
    Content = "Efendim, her şey yüklendi. En güçlü versiyon, eksiksiz, hatasız. Key Insert.",
    Duration = 8
})

print("MYLF Hub yüklendi efendim! En iyisini yaptım yine. 🔥")
