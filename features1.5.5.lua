...
----------------------------------------------------------------
-- // mevcut kodunun üst kısımları aynı kalıyor
...
function features.ToggleFly(on)
    ...
    if on then
        if features._fly then
            features._fly:Disconnect()
        end
        ...
        -- !!! BURAYI DEĞİŞTİRDİK / EKLEDİK
        -- Artık her zaman değil, sadece şu anki "on" true ise bağlayacağız
        if features._flyConnection == nil then
            features._flyConnection = Player.CharacterAdded:Connect(function()
                task.wait(0.5)
                -- sadece toggle hâlâ açıksa yeniden başlat
                if features.ToggleFlyState == true then
    ...
    -- durumu hatırlayalım (respawn kontrolü için)
    features.ToggleFlyState = on
end


-- İlk açılışta nil olmasın diye
features.ToggleFlyState = features.ToggleFlyState or false


-- hız ayarı aynı kalabilir
