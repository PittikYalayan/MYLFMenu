...
-- Aimbot: açı tabanlı (mesafeden bağımsız hedef seçimi) + opsiyonel auto-fire + smoothness
-- ESP: join/leave/respawn canlı, Players destekli
-- Noclip: restore’lu Transparency
-- Fly: LCtrl aşağı
...
features.Smoothness      = 0.1      -- 0 = anında bak; 1 = yumuşak (menüdeki gibi slider uyumlu)
...
features.Prediction      = false    -- prediction toggle
features.PredictionAmount = 0.135   -- prediction amount
...
local teleportCooldown = 1 -- saniye
...
function features.ToggleTeleport(on)
    ...
    if on then
        ...
        features._tp = RunService.Heartbeat:Connect(function()
            ...
            if UIS:IsKeyDown(Enum.KeyCode.T) and (currentTime - lastTeleportTime) >= teleportCooldown then
...
function features.ToggleAimbot(on)
    ...
    local function step()
        ...
        local pred = features.Prediction and head.Velocity * features.PredictionAmount or Vector3.new()
        local target = CFrame.new(cam.CFrame.Position, head.Position + pred)
        local s = tonumber(features.Smoothness) or 0.1
        cam.CFrame = (s > 0.1) and cam.CFrame:Lerp(target, 1-s) or target
...
function features.ToggleRageMode(on)
        ...
        features._autoBehindConn = RunService.RenderStepped:Connect(function()
            ...
            if target and target.Parent then
                ...
                if enemyHRP and myHRP then
                    ...
                    -- Arkasına pozisyon (3 stud arkası (bazen aşağı kaydırma)
...
function features.ToggleRainbowName(on)
    ...
    elseif not on and features._conns.rainbow then
        ...
        for _,o in pairs(features._targets) do
            if o.label then 
                o.label.TextColor3=Color3.fromRGB(255,255,255)
                if o.billboard then o.billboard:Destroy() end -- OFF yapınca tamamen sil
            end
        end
    end
end
