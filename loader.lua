-- ⚡ MYLF Premium Loader (Linoria) - Modern Design v3 (Siyah & Kırmızı Tema)
...
-- === Tema (Siyah & Kırmızı) ===
Library.BackgroundColor = Color3.fromRGB(15, 15, 15)      -- Siyah
Library.MainColor       = Color3.fromRGB(120, 0, 0)       -- Koyu kırmızı
Library.AccentColor     = Color3.fromRGB(255, 0, 0)       -- Parlak kırmızı
Library.OutlineColor    = Color3.fromRGB(40, 0, 0)        -- Bordo outline
Library.FontColor       = Color3.fromRGB(220, 220, 220)   -- Açık gri yazı
...
RageGroup:AddToggle('RageWallCheck', {Text = 'Wall Check'}):OnChanged(function(val) Features.SetWallCheck(val) end)
RageGroup:AddToggle('RageTeamCheck', {Text = 'Team Check'}):OnChanged(function(val) Features.SetTeamCheck(val) end)
RageGroup:AddSlider('RageSmooth', {Text = 'Smoothness', Min = 1, Max = 10, Default = 5, Rounding = 1}):OnChanged(function(val) Features.SetSmoothness(val) end)
RageGroup:AddToggle('RagePrediction', {Text = 'Prediction'}):OnChanged(function(val) Features.SetPrediction(val) end)
RageGroup:AddToggle('RageSticky', {Text = 'Sticky Aim'}):OnChanged(function(val) Features.SetSticky(val) end)
RageGroup:AddKeyPicker('RageAimKey', {
    Default = 'MouseButton2', Mode = 'Hold', Text = 'Aimbot Key',
...
PlayerGroup:AddKeyPicker('FlyKey', {Default = 'F', Mode = 'Toggle', Text = 'Fly Key', Callback = function(val) Features.SetFlyKey(val) end})

-- Speed
PlayerGroup:AddToggle('SpeedEnabled', {Text = 'Enable Walk Speed'}):OnChanged(function(val) Features.ToggleSpeed(val) end)
PlayerGroup:AddSlider('SpeedValue', {Text = 'Walk Speed', Min = 16, Max = 300, Default = 16}):OnChanged(function(val) Features.SetSpeed(val) end)
PlayerGroup:AddKeyPicker('SpeedKey', {Default = 'LeftShift', Mode = 'Hold', Text = 'Speed Key', Callback = function(val) Features.SetSpeedKey(val) end})
...
PlayerGroup:AddKeyPicker('InfJumpKey', {Default = 'Space', Mode = 'Hold', Text = 'Infinite Jump Key', Callback = function(val) Features.SetInfiniteJumpKey(val) end})
